...
export default function Home() {
  ...
  const handleGetStarted = async () => {
    ...
    if (restaurantName.trim()) {
      router.push(
        `/register?restaurantName=${encodeURIComponent(restaurantName)}`
      );
    } else {
            ...
            <div className="flex flex-col gap-8 z-10">
              <div className="flex flex-col gap-6 items-center">
                ...
                <p className="text-white text-sm md:text-base font-medium text-center">
                  Experience easy, cost-free restaurant management with zero
                  setup hassle
                </p>
              </div>
              <div className="flex flex-col md:flex-row gap-3 md:gap-2">
                ...
                <div className="w-full md:w-[145px]" onClick={handleGetStarted}>
                  <PrimaryBtn text={isLoading ? "Loading..." : `Get Started Now`} />
                </div>
              </div>
            </div>
          </div>
